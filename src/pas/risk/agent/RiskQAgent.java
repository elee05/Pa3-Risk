    ...
    extends NeuralQAgent
{
     ...
     */
    @Override
    public Action getExplorationRedeemAction(final GameView game,final int actionCounter,final boolean canRedeemCards) {
        final List<Action> options = this.getRedeemActions(game, actionCounter, canRedeemCards, true);
        if (options == null || options.isEmpty()) {
            // fall back to attack/redeem actions if no pure redeem options exist
            return chooseRandom(this.getAttackRedeemActions(game, actionCounter, canRedeemCards), new Random());
        }
        System.out.println("Redeem options size: " + options.size());
        for (Action a : options) {
            System.out.println("  option: " + a.getClass().getSimpleName());
        }
     ...
     */
    @Override
    public boolean shouldExploreRedeemMovePhase(final GameView game,final int actionCounter,final boolean canRedeemCards) {
